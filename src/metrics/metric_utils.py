...
class MetricOptions:
    def __init__(self, G=None, G_kwargs={}, dataset_kwargs={}, num_gpus=1,
                 rank=0, device=None, progress=None, cache=True):
...
class ProgressMonitor:
    def __init__(self, tag=None, num_items=None, flush_interval=1000, verbose=False,
                 progress_fn=None, pfn_lo=0, pfn_hi=1000, pfn_total=1000):
        self.tag = tag
    ...
    def update(self, cur_items):
        ...
        if (self.verbose) and (self.tag is not None):
            print(f'{self.tag:<19s} items {cur_items:<7d} time '
                  f'{dnnlib.util.format_time(total_time):<12s} ms/item {time_per_item * 1e3:.2f}')
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1,
    ...
    if opts.cache:
        ...
        args = dict(dataset_kwargs=opts.dataset_kwargs, detector_url=detector_url,
    ...
    try:
        ...
        item_subset = [(i * opts.num_gpus + opts.rank) % num_items
                       for i in range((num_items - 1) // opts.num_gpus + 1)]
        for images_0, images_1, _, _, _ in torch.utils.data.DataLoader(
                dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs
        ):
            if images_0.shape[1] == 1:
                images_0 = images_0.repeat([1, 3, 1, 1])
                images_1 = images_1.repeat([1, 3, 1, 1])

            # video case
            if len(images_0.shape) == 5:
                images_0 = rearrange(images_0, 'b c t h w -> (b t) c h w')
                images_1 = rearrange(images_1, 'b c t h w -> (b t) c h w')
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1,
    ...
    def run_generator(z, c):
        try:
            img, _ = G(z=z, c=c, timesteps=2, **opts.G_kwargs)
        except:
            img, _ = G(z=z, c=c, **opts.G_kwargs)
