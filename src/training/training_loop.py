...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    # Load data.
    images_0, images_1, labels_0, labels_1, _ = zip(*[training_set[i] for i in grid_indices])
    return (gw, gh*2), np.concatenate((np.stack(images_0), np.stack(images_1)), axis=0), np.stack(labels_0)
...
def training_loop(
    ...
    common_kwargs = dict(c_dim=training_set.label_dim, img_resolution=training_set.resolution, img_channels=training_set.num_channels, ours=True)
    ...
    if rank == 0:
        ...
        img, Ts = misc.print_module_summary(G, [z, c, 2])
        img = rearrange(img, '(b t) c h w -> b (t c) h w', t=2)
        Ts = Ts.repeat(1, 1, img.size(-2), img.size(-1))
    ...
    if rank == 0:
        ...
        images = rearrange(images, '(t b) c h w -> (b t) c h w', t = 2)
        save_image_grid(images, os.path.join(run_dir, 'reals.jpg'), drange=[0,255], grid_size=grid_size)
        grid_z = torch.randn([labels.shape[0], G.z_dim], device=device).split(batch_gpu)
        grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
        images = torch.cat([G_ema(z=z, c=c, timesteps=2, noise_mode='const')[0].cpu() for z, c in zip(grid_z, grid_c)]).numpy()
        images = convert_batch(images)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            phase_real_img = torch.cat([phase_real_img_0, phase_real_img_1], dim=1)
            phase_real_c = phase_real_c_0  # should be identical
            # phase_real_c   = torch.cat([phase_real_c_0, phase_real_c_1])

            phase_real_img = (phase_real_img.to(device).to(torch.float32) / 127.5 - 1)

            times = times.view(-1, 1, 1, 1).to(device)
            times = times * torch.ones_like(phase_real_img[:, 0:1])
            phase_real_img = torch.cat([phase_real_img, times], dim=1).split(batch_gpu)
