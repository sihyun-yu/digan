...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        if self.cfg.fmm.enabled:
            x = fmm_modulate_linear(x=x, weight=self.weight, styles=styles, noise=noise,
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        if in_channels == 0:
            ...
            if self.cfg.coords.enabled and (
                    not self.cfg.coords.per_resolution or self.resolution > self.input_resolution):
            ...
            self.conv0 = SynthesisLayer(conv0_in_channels, out_channels, w_dim=w_dim, resolution=self.resolution,
                                        up=up_for_conv0,
        ...
        self.conv1 = SynthesisLayer(conv1_in_channels, out_channels, w_dim=w_dim, resolution=self.resolution,
                                    ...
                                    cfg=cfg,
                                    instance_norm=(in_channels > 0 and cfg.get('fmm', {}).get('instance_norm', False)),
                                    **layer_kwargs)
        self.num_conv += 1
    ...
    def forward(self, x, img, ws, Ts=None, z_motion=None, force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if self.in_channels == 0:
            conv1_w = next(w_iter)
            x = self.input(ws.shape[0], conv1_w, Ts, z_motion, device=ws.device, dtype=dtype,
        ...
        if self.in_channels == 0:
            ...
            if self.coord_fuser is not None:
                x = self.coord_fuser(x, conv0_w, Ts, z_motion, dtype=dtype, memory_format=memory_format)
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, resolution=res,
                                   img_channels=img_channels, is_last=is_last, use_fp16=use_fp16, cfg=cfg,
                                   **block_kwargs)
    ...
    def forward(self, ws, Ts=None, z_motion=None, **block_kwargs):
        ...
        x = img = None
        timesteps = Ts.size(0) // z_motion.size(0)

        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f'b{res}')
            cur_ws = cur_ws.unsqueeze(1).repeat(1, timesteps, 1, 1).view(-1, cur_ws.size(1), cur_ws.size(2))
            x, img = block(x, img, cur_ws, Ts, z_motion, **block_kwargs)
    ...
    def forward(self, z, c, timesteps=None, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        ...
        if timesteps != None:
            ...
            if timesteps > 2:
                ...
                Ts = Ts.repeat(batch_size, 1, 1, 1).view(-1, 1, 1, 1).to(z.device)
                dT = None

            else:
                Ts = torch.cat([self.dist1.sample((batch_size, 1, 1, 1, 1)),
                                self.dist2.sample((batch_size, 1, 1, 1, 1))], dim=1).to(z.device)
...
@persistence.persistent_class
class OurDiscriminator(torch.nn.Module):
    def __init__(self,
                 c_dim,  # Conditioning label (C) dimensionality.
                 img_resolution,  # Input resolution.
                 img_channels,  # Number of input color channels.
        ...
        super().__init__()
        self.videoD = ImageDiscriminator(c_dim, img_resolution, 2 * img_channels + 1, **kwargs)
        self.imageD = ImageDiscriminator(c_dim, img_resolution, img_channels, **kwargs)

    def forward(self, vid, c, **block_kwargs):
        img = rearrange(vid[:, :-1], 'b (t c) h w -> (b t) c h w', t=2)
        cc = rearrange(c.unsqueeze(1).repeat(1, 2, 1), 'b t c -> (b t) c')  # repeat
        x_vid = self.videoD(vid, c, **block_kwargs)
        x_img = self.imageD(img, cc, **block_kwargs)
        return x_vid, x_img, img
