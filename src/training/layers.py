...
class GenInput(nn.Module):
    ...
    def forward(self, batch_size: int, w: Tensor = None, Ts: Tensor = None, z_motion: Tensor = None, device=None,
                dtype=None, memory_format=None) -> Tensor:
        if self.cfg.type == 'const':
            x = self.input.to(dtype=dtype, memory_format=memory_format)
            x = x.unsqueeze(0).repeat([batch_size, 1, 1, 1])
        elif self.cfg.type == 'coords':
            x = self.input(batch_size, w, Ts, z_motion, device=device, dtype=dtype, memory_format=memory_format)
        else:
            raise NotImplementedError
...
class CoordsInput(nn.Module):
    ...
    def forward(self, batch_size: int, w: Optional[Tensor] = None, Ts: Optional[Tensor] = None,
                z_motion: Optional[Tensor] = None, device='cpu', dtype=None, memory_format=None) -> Tensor:
        ...
        out = self.coord_fuser(dummy_input, w, Ts=Ts, z_motion=z_motion, dtype=dtype, memory_format=memory_format)
...
class CoordFuser(nn.Module):
    ...
    def __init__(self, cfg: DictConfig, w_dim: int, resolution: int):
        ...
        self.motion_sigma = self.cfg.get('motion_sigma', 0.25)
        ...
        if self.log_emb_size > 0:
            self.register_buffer('log_basis', generate_logarithmic_basis(
                ...
                use_diagonal=self.res_cfg.get('use_diagonal', False)))  # [log_emb_size, 2]
        ...
        if self.shared_emb_size > 0:
            ...
            self.affine = FullyConnectedLayer(w_dim, self.shared_emb_size, bias_init=0)
        ...
        if self.predictable_emb_size > 0:
            ...
            self.xyaffine = FullyConnectedLayer(w_dim, self.W_size + self.b_size, bias_init=0)

            self.affine = FullyConnectedLayer(w_dim * 2, self.b_size, bias_init=0)
            self.motion_mlp = nn.Sequential(
                FullyConnectedLayer(self.b_size, self.b_size, activation='lrelu'),
                FullyConnectedLayer(self.b_size, self.b_size)
            )
    ...
    def forward(self, x: Tensor, w: Tensor = None, Ts: Tensor = None, z_motion: Tensor = None, dtype=None,
        ...
        if (not self._fourier_embs_cache is None) and (self._fourier_embs_cache.device == x.device) and \
            ...
            if self.shared_emb_size > 0:
                ...
                if Ts != None:
                    # Currently, do not use z_motion
                    with torch.autograd.profiler.record_function('motion'):
                        motion_vec = self.affine(w).view(batch_size, self.shared_emb_size, 1, 1)
                        bias = motion_vec * Ts * self.fourier_scale

                raw_embs.append(raw_shared_embs + bias)

            if self.predictable_emb_size > 0:
                misc.assert_shape(w, [batch_size, None])
                mod = self.xyaffine(w)  # [batch_size, W_size + b_size]
                W = self.fourier_scale * mod[:, :self.W_size]  # [batch_size, W_size]
                W = W.view(batch_size, self.predictable_emb_size,
                           self.cfg.coord_dim)  # [batch_size, predictable_emb_size, coord_dim]
                bias = mod[:, self.W_size:]  # [batch_size, predictable_emb_size]

                if Ts != None:
                    with torch.autograd.profiler.record_function('motion'):
                        time_step = w.size(0) // z_motion.size(0)
                        z_motion = z_motion.unsqueeze(1).repeat(1, time_step, 1)  # 2 is timestep
                        z_motion = rearrange(z_motion, 'b t d -> (b t) d', t=time_step)  # 2 is timestep
                        w = torch.cat([w, z_motion], dim=1)

                        motion_vec = self.affine(w).view(batch_size, self.predictable_emb_size, 1, 1)
                        motion_vec = F.leaky_relu(motion_vec * Ts * self.motion_sigma)
                        motion_vec = self.motion_mlp(motion_vec.view(batch_size, self.predictable_emb_size))
                        bias = bias + motion_vec

                bias = bias.view(batch_size, -1, 1, 1)  # expand dim to add for all (H, W)

                raw_predictable_embs = (torch.einsum('bdc,bcxy->bdxy', W, raw_coords) + bias)
                raw_embs.append(raw_predictable_embs)

            if len(raw_embs) > 0:
                raw_embs = torch.cat(raw_embs, dim=1)
                raw_embs = raw_embs.contiguous()  # [batch_size, -1, img_size, img_size]
